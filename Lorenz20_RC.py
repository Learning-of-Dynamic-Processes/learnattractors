# In[RC multi-step ahead]

filename_rc = "orth_noisy_filt8.h5"
NN1orth = buildNN(-20,20)
NN1orth.load_weights(filename_rc)
fc_length = 1000
burnin = 100001
start_time = burnin + 3000
fc_orth1_x = np.zeros(fc_length+start_time,)
for i in range(fc_length):
    if i == 0:
        xt = data_input_rc_states[i + start_time - burnin,:]
        fc_orth1_x[i+start_time] = NN1orth.predict(xt.reshape(1,d)).flatten()
    else:
        xt = Aorth@xt + Crc*fc_orth1_x[i-1+start_time]
        fc_orth1_x[i+start_time] = NN1orth.predict(xt.reshape(1,d)).flatten()
   
mse_fc_rc_x_1 =(fc_orth1_x[start_time:fc_length+start_time]-data_target[0][start_time-burnin:fc_length+start_time-burnin])**2
mmse_fc_rc_x_1 = np.mean(mse_fc_rc_x_1)

fc_length = 1000
fig, (ax1, ax2, ax3) = plt.subplots(3, 1)
fig.set_size_inches(10, 11)
fontsizeval=30
#fig.suptitle("RC NN: Pseudo-Multistep Insample Forecast x axis", fontsize=30)
ax1.plot(data_target[0][start_time-burnin:fc_length+start_time-burnin],linewidth=3)
ax1.set_xlabel("Lorenz x-axis", fontsize=fontsizeval)
ax1.tick_params(axis='both', which='major', labelsize=fontsizeval)
ax2.plot(fc_orth1_x[start_time:fc_length+start_time],linewidth=3)
ax2.set_xlabel("x-forecast", fontsize=fontsizeval)
ax2.tick_params(axis='both', which='major', labelsize=fontsizeval)
ax3.plot(mse_fc_rc_x_1,linewidth=3)
ax3.set_xlabel("mse", fontsize=fontsizeval)
...
print(mmse_fc_rc_x_1)

# In[RC one-step ahead]
...
for i in range(fc_length):
    if i==0:
        ...
        xt = Aorth@xt + Crc*LS_noisy_x[i-1+start_time]#   Crc*fc_orth1_x[i-1+start_time]
        fc_orth1_x[i+start_time] = NN1orth.predict(xt.reshape(1,d)).flatten()
   
mse_1stepfc_rc_x_1 =(fc_orth1_x[start_time:fc_length+start_time]-data_target[0][start_time-burnin:fc_length+start_time-burnin])**2
mmse_1stepfc_rc_x_1 = np.mean(mse_1stepfc_rc_x_1)
...
ax3.scatter(t[start_time:plotlength+start_time],fc_orth1_x[start_time:plotlength+start_time],color = cmap, s = 5,linewidth=3)
ax3.title.set_size(labelfontsizeval)
ax3.title.set_text(r"one-step ahead forecast of the $u$-component")
ax3.tick_params(axis='both', which='major', labelsize=fontsizeval)
ax4.plot(mse_1stepfc_rc_x_1,linewidth=3)
...
plt.yticks(fontsize=fontsizeval)
plt.show()
fig.savefig('Lorenz_traj_ucomponent.png', format='png', dpi=300)

print(mmse_1stepfc_rc_x_1)
...
# Plot the result of RC+NN
fig = plt.figure(figsize = (10,10))
#fig.suptitle("A filtered and one-step forecasted trajectory")
ax = fig.gca(projection='3d')
ax.scatter(fc_orth1_x[start_time:plotlength+start_time], LS_y[start_time:plotlength+start_time], LS_z[start_time:plotlength+start_time], color = cmap, s = 5)
ax.set_xlabel("$u$", fontsize=midfontsizeval)
ax.set_ylabel("$v$", fontsize=midfontsizeval)
ax.set_zlabel("$w$", fontsize=midfontsizeval)
ax.tick_params(axis='both', which='major', labelsize=fontsizeval)
ax.view_init(15, -50)
fig.savefig('Lorenz_traj_filtfrcst.png', format='png', dpi=300)
plt.show()
# In[in-sample training: verification]:

#in-sample
NN1orth = buildNN(-20,20)
NN1orth.load_weights(filename_rc)
LS_rc_x_is = NN1orth.predict(data_input_rc_states)
mse_is_rc_x_1 =(LS_rc_x_is[start_time - burnin:fc_length+start_time - burnin].T-data_target[0][start_time - burnin:fc_length+start_time - burnin])**2
mse_is_rc_x_1 = mse_is_rc_x_1[0]
mmse_is_rc_x_1 = np.mean(mse_is_rc_x_1)
...
for t in range(3000,10000,10):
    ...
    mse_fc_rc_x_1 =(fc_orth1_x[start_time:fc_length+start_time]-data_target[0][start_time - burnin:fc_length+start_time - burnin])**2
    mmse_fc_rc_x_1[ind] = np.mean(mse_fc_rc_x_1)
    ind += 1

plt.plot(mmse_fc_rc_x_1)
mean_mmse_fc_rc_x_1 = np.mean(mmse_fc_rc_x_1)
print(mean_mmse_fc_rc_x_1)
